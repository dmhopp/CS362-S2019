...
//ASSIGN2 BUG: Altered while loop range so that last card in temphand is not discarded
...
//ASSIGN2 BUG: Guarantees the second tribute card will always be dropped by comparing the first card to itself when checking for duplicates
int refactorTribute(struct gameState *state){
      int i;
      int currentPlayer = whoseTurn(state);
	...
	state->deckCount[nextPlayer]--;
      ...
      //ORIGINAL: if(tributeRevealedCards[0] == tributeRevealedCards[1])		       
...
//ASSIGN2 BUG: Four cards will be added to hand instead of three
int refactorSmithy(struct gameState *state, int handPos){
      int i;
      int currentPlayer = whoseTurn(state);

      //+3 Cards
      for (i = 0; i <= 3; i++) //ORIGINAL: for(i = 0; i < 3; i++)
	{
	  drawCard(currentPlayer, state);
	}
...
int refactorSeaHag(struct gameState *state){
      int i;
      int currentPlayer = whoseTurn(state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      return refactorAdventurer(state);
      /*while(drawntreasure<2){
    ...
    case gardens:
      refactorGardens(); //ORIGINAL: return refactorGardens();
      //return -1;
    ...
    case smithy:
      return refactorSmithy(state, handPos);
      /*//+3 Cards
      for (i = 0; i < 3; i++)
	{
	  drawCard(currentPlayer, state);
